...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, generateMagicEdit, generateImageFromPrompt } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
        const editedImageUrl = await generateEditedImage(currentImage, retouchPrompt, editHotspot);
  ...
  }, [currentImage, retouchPrompt, editHotspot, addImageToHistory]);
  ...
  const handleMagicGenerate = useCallback(async () => {
    ...
    try {
        const editedImageUrl = await generateMagicEdit(currentImage, magicPrompt);
        const newImageFile = dataURLtoFile(editedImageUrl, `magic-edited-${Date.now()}.png`);
        addImageToHistory(newImageFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`Failed to generate the image. ${errorMessage}`);
  ...
  }, [currentImage, magicPrompt, addImageToHistory]);
    ...
    if (isLoading && !currentImageUrl) {
      return (
        <div className="text-center animate-fade-in flex flex-col items-center justify-center gap-4">
            <Spinner />
            <p className="text-gray-300 text-lg">Generating your image, please wait...</p>
            <p className="text-gray-400">This can take a moment for high-quality generation.</p>
        </div>
      );
    }
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['magic', 'retouch', 'adjust', 'filters', 'crop'] as Tab[]).map(tab => (
                 <button
                    key={tab}
            ...
            {activeTab === 'magic' && (
                <div className="flex flex-col items-center gap-4 animate-fade-in">
                    <p className="text-md text-gray-400">
                        Describe any change you want to make to the image. Be creative!
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleMagicGenerate(); }} className="w-full flex items-center gap-2">
                        <input
                            type="text"
                            value={magicPrompt}
